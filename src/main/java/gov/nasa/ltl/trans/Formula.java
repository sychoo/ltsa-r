...
public class Formula implements Comparable<Formula> {
  ...
  private static Character[] atomCharsAr = {':','.','='};
  private static Vector<Character> atomChars = new Vector(Arrays.asList(atomCharsAr));
  ...
  private static Formula parse (Input i, int precedence)
    ...
    try {
      ...
      case ')':
        throw new ParseErrorException("first invalid character: " + ch);
      ...
      case '(':
        ...
        if (i.get() != ')') {
          throw new ParseErrorException("second invalid character: " + ch);
        }
      ...
      default:
        ...
        if (Character.isJavaIdentifierStart(ch) || atomChars.contains(ch)) {
          ...
          try {
            while ((Character.isJavaIdentifierPart(ch = i.get()) || atomChars.contains(ch)) &&
          ...
          throw new ParseErrorException("third invalid character: " + ch);
        }
      ...
      while (true) {
        ...
        case '(':
        default:
          throw new ParseErrorException("fourth invalid character: " + ch);
        }
