...
public class Minimiser {
    ...
    private CompactState machTau(CompactState m) {
        ...
        for (int i=0; i<m.states.length; i++)
            m.states[i] = EventState.add(m.states[i],new EventState(Declaration.TAU,i));
        output.out(".");
        return m;
    }
    ...
    */
    ...
    public CompactState minimise() {
         ...
         if (machine.endseq>=0) {
            ...
            machine.states[es] 
             = EventState.add(machine.states[es],new EventState(machine.alphabet.length,es));
         }
         if (machine.hasTau()) {
             initTau();
    ...
    */
    ...
    private CompactState makeNewMachine() {
        ...
        for (int i = 0; i<machine.maxStates; i++) {
            ...
            EventState tmp = EventState.renumberStates(machine.states[i],oldtonew);
