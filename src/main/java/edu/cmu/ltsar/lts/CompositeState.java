...
public class CompositeState {
    ...
    public boolean makeControl = false; // make a controller
    ...
    public StateLabels labels;     // controller labelled by fluents
    ...
    public void compose(LTSOutput output, boolean ignoreAsterisk) {
        if(machines!=null && machines.size()>0) {
            ...
            if (makeDeterministic) {
                ...
                composition.makeController(hidden,output);
                machines = lts.ltl.FluentProcess.getFluents();
                if (machines==null) {
                    output.outln("No fluents defined: controller build -  reactive controller not built");
                    return;
                }
                machines.add(0,composition);
                a = new Analyser(this,output,null,ignoreAsterisk);
                labels = new StateLabels(composition, a.stateLabel(),a.getCoder(), lts.ltl.FluentProcess.getNames());
            } else
                applyHiding();
        }
    }
