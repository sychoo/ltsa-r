...
public class PredicateDefinition {
	...
	if (p.range == null)  {
            ...
            FluentProcess.make(p.name,p.initial,p.trueActions,p.falseActions);
        } else  {
		...
		String newName = p.name+"."+s;
                PredicateDefinition pp = new PredicateDefinition(newName,PA,NA,init);
                FluentProcess.make(pp.name,pp.initial,pp.trueActions,pp.falseActions);
		definitions.put(newName,pp);
            }
