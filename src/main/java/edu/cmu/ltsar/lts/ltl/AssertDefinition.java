...
public class AssertDefinition {
    ...
    private AssertDefinition(Symbol n, FormulaSyntax f, LabelSet ls, Hashtable ip, Vector p, boolean b){
    	  name = n;
    	  ltl_formula = f;
    	  cached = null;
          alphaExtension = ls;
    ...
    public static void put(Symbol n, FormulaSyntax f, LabelSet ls, Hashtable ip, Vector p, boolean isConstraint, boolean isProperty) {
	    ...
	    if (!isConstraint)  {
    	       if(definitions.put(n.toString(),new AssertDefinition(n,f,ls, ip, p, false))!=null) 
                 Diagnostics.fatal ("duplicate LTL property definition: "+n, n);
    	    } else  {
                 if(constraints.put(n.toString(),new AssertDefinition(n,f,ls, ip, p, isProperty))!=null) 
	...
	public static CompactState compileConstraint(LTSOutput output, Symbol name, String refname, Vector pvalues)  {
		...
		if (!p.isProperty) cs.composition.unMakeProperty();
