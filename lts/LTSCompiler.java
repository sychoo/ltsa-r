       ...
       while (current.kind != Symbol.EOFSYM) {
          if (current.kind ==Symbol.CONSTANT) {
		   ...
		   next_symbol();
              ...
              if (current.kind == Symbol.SAFE)      { makeControl = true; next_symbol();}
...
private void assertDefinition(boolean isConstraint, boolean isProperty) {
	...
	AssertDefinition.put(name,f,ls,initparams,params,isConstraint, isProperty);
}
