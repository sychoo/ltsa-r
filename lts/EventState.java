...
public class EventState {
    ...
    public static boolean hasUnControlledTransToError(EventState head, BitSet uncontrolled) {
        EventState p =head;
        while(p!=null) {
            EventState q = p;
            while (q!=null) {
                if (uncontrolled.get(q.event) && q.next == Declaration.ERROR) return true;
                q=q.nondet;
            }
            p=p.list;
        }
    ...
    public static boolean setStateError(EventState head, BitSet errorStates) {
        EventState p =head;
        while(p!=null) {
            EventState q = p;
            while (q!=null) {
                if (q.next!=Declaration.ERROR){
                    if (errorStates.get(q.next)) q.next=Declaration.ERROR;
                }
                q=q.nondet;
            }
            p=p.list;
   ...
   public static boolean hasEvent(EventState head, int event) {
    ...
    public static void checkNotDetControls(EventState head, BitSet uncontrolled, String[] alpha) {
        EventState p =head;
        while(p!=null ) {
             if (!uncontrolled.get(p.event) && p.nondet!= null) {
                 Diagnostics.fatal("non-deterministic control for action:- " + alpha[p.event]);
             }
             p=p.list;
        }
    }
    ...
    public static boolean hasEventInSet (EventState head, BitSet actions){
        EventState p =head;
        while(p!=null) {
             if (actions.get(p.event)) return true;
             p=p.list;
        }
        return false;
    }
	...
	public static EventState removeTransToState(EventState head, int statenumber)  {
	  EventState p =head;
	  EventState newHead = null;
	  while(p!=null) {
	       if (p.next != statenumber) 
